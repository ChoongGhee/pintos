...
#define F (1 << 14)																// 고정소수점의 스케일(여기서는 2^14를 사용)
#define INT_TO_FP(n) ((n) * F)													// Convert n to fixed point: n * f
#define FP_TO_INT_ZERO(x) ((x) / F)												// Convert x to integer (rounding toward zero): x / f
#define FP_TO_INT_NEAREST(x) ((x) >= 0 ? ((x) + F / 2) / F : ((x) - F / 2) / F) // Convert x to integer (rounding to nearest)
#define FP_ADD(x, y) ((x) + (y))												// Add x and y: x + y
#define FP_SUB(x, y) ((x) - (y))												// Subtract y from x: x - y
#define FP_ADD_INT(x, n) ((x) + (n) * F)										// Add x and n: x + n * f
#define FP_SUB_INT(x, n) ((x) - (n) * F)										// Subtract n from x: x - n * f
#define FP_MUL(x, y) (((int64_t)(x)) * (y) / F)									// Multiply x by y: ((int64_t) x) * y / f
...
// 재원 추가
// static void less_fun(struct list_elem *elem, struct list_elem *e, void *aux);
...
void thread_awake(int current_ticks)
{
	...
	for (temp = list_begin(&sleeping_list); temp != list_end(&sleeping_list);)
	{
		struct thread *t = list_entry(temp, struct thread, elem);

		if (t->sleep_tick <= current_ticks)
		{
...
void thread_sleep(int ticks)
{
	...
	temp->sleep_tick = ticks;
	list_insert_ordered(&sleeping_list, &temp->elem, &thread_less_fun, (void *)offsetof(struct thread, sleep_tick));
...
void thread_unblock(struct thread *t)
{
	...
	if (thread_mlfqs)
	{
		...
		list_insert_ordered(&ready_list, &t->elem, &thread_greater_fun, (void *)offsetof(struct thread, priority));
	}
	else
	{
		// 재원 추가
...
/** 현재 실행 thread를 반환. but pop처럼 리스트에서 나오지는 않음. */
struct thread *
thread_current(void)
{
	// 현재 실행 thread 반환
...
/** current thread를 종료시킴. 인터럽트도 비활성화 */
...
// 양보하다 : yield  In programming : 프로그램의 제어를 넘겨준다.

/**스케쥴러 리스트에 있는 스레드를 꺼내 작업함. */
void thread_yield(void)
{
	...
	if (curr != idle_thread)
	{
		...
		{
			// 재원 추가 mlfqs
			// list_push_back(&all_list, &curr->all_elem);
...
/* Idle thread.  Executes when no other thread is ready to run.
   ...
   아이들 스레드. 다른 실행 준비된 스레드가 없을 때 실행됩니다.
	아이들 스레드는 처음에 thread_start()에 의해 준비 리스트에 올려집니다.
	초기에 한 번 스케줄링되며, 이때:

	1. idle_thread를 초기화하고,
	2. thread_start()가 계속 진행될 수 있도록 전달받은 세마포어를 "up"하고
	3. 즉시 차단됩니다.

	그 이후로 아이들 스레드는 준비 리스트에 다시 나타나지 않습니다.
	준비 리스트가 비어있을 때 next_thread_to_run()에 의해 특별한 경우로 반환됩니다.
   */
// idle이란 유휴라는 뜻
// 컴퓨터 시스템이 사용 가능한 상태이나 실제적인 작업이 없는 시간.
// 계산 처리 시간과 데이터 입출력 처리 시간의 차이 등으로 컴퓨터의 어느 한쪽이 대기 상태에 있는 경우이다.
static void
...
{
	if (t == idle_thread) // idle_thread는 우선순위를 계산하지 않음
		return;

	int new_priority = FP_TO_INT_NEAREST(INT_TO_FP(PRI_MAX) - ((t->recent_cpu / 4) + INT_TO_FP(t->nice * 2)));
...
void all_prior_set(void)
{
	...
	// 모든 스레드에 대해 recent_cpu와 priority를 재계산
